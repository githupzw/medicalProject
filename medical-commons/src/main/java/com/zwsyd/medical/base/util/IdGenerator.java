package com.zwsyd.medical.base.util;
 ...
 *</p>
 *<p>
 *  使用 Snowflake 算法生成的 64位 long 类型的 ID，结构如下:<br/>
 *  0-0000000000 0000000000 0000000000 0000000000 0-00000-00000-000000000000<br/>
 *  (1)01位标识，由于 long 在 Java 中是有符号的，最高位是符号位，正数是 0，负数是 1，ID 一般使用正数，所以最高位是 0<br/>
 *  (2)41位时间戳(毫秒级)，注意，41位时间戳不是存储当前时间的时间戳，而是存储时间戳的差值(当前时间-开始时间)得到的值，
 *      开始时间戳，一般是业务开始的时间，由我们程序来指定，如 IdWorker 中的 START_TIMESTAMP 属性。
 *      41位的时间戳，可以使用 70年:(2^41)/(1000*60*60*24*365)=69.7年<br/>
 *  (3)10位的数据机器位，可以部署在 1024个节点，包括 5位 datacenterId 和 5位 workerId<br/>
 *  (4)12位序列，毫秒内的计数，12位的计数顺序号支持每个节点每毫秒(同一机器，同一时间戳)产生 4096个 ID 序号<br/>
 *</p>
 *<p>
 *  SnowFlake 的优点是，整体上按照时间自增排序，并且整个分布式系统内不会产生 ID 碰撞(由数据中心 ID 和机器 ID 作区分)，并且效率较高。
 *  最多支持 1024台机器，每台机器每毫秒能够生成最多 4096个 ID，整个集群理论上每秒可以生成 1024*1000*4096=42亿个 ID。
 *</p>
 * @author ZhangWei
 * @version 1.0.0
 * @since 1.0.0
